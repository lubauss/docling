...
def verify_table(true_item: TableItem, pred_item: TableItem, fuzzy: bool):
...
def verify_docitems(doc_pred: DoclingDocument, doc_true: DoclingDocument, fuzzy: bool):
    ...
    for (true_item, _true_level), (pred_item, _pred_level) in zip(
        ...
        if isinstance(true_item, TableItem):
            ...
            assert verify_table(true_item, pred_item, fuzzy=fuzzy), (
                "Tables not matching"
            )
        ...
        if isinstance(true_item, PictureItem):
            ...
            if true_image is not None:
                assert verify_picture_image(true_image, pred_image), (
...
def verify_conversion_result_docling_parse(
    input_path: Path,
    ...
    gt_subpath = input_path.parent / "groundtruth" / "docling" / input_path.name
    if str(input_path.parent).endswith("pdf"):
        gt_subpath = (
            input_path.parent.parent / "groundtruth" / "docling" / input_path.name
        )
